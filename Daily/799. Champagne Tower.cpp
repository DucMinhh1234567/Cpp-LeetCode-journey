...
// The glass indexed as (1, 0) and the glass indexed as (1, 1) 
...
class Solution {
    public:
        double champagneTower(int poured, int query_row, int query_glass) {
            double stack[102][102] = {0.0};
            stack[0][0] = poured;
            for (int row = 0; row <= query_row; row++){
                for (int glass = 0; glass <= row; glass++){
                    if (stack[row][glass] > 1){
                        double excess = (stack[row][glass] - 1) / 2.0;
                        stack[row][glass] = 1.0;
                        stack[row + 1][glass] += excess;
                        stack[row + 1][glass + 1] += excess;
                    }
                }
            }
